...
private:
    ...
    inline void modbuserror_handle(const uint8_t *msg, int func);
...
public:
    ...
    inline modbus() = default; //TODO: might need to change this.
    inline modbus(std::string host, uint16_t port);
    inline ~modbus();

    inline bool modbus_connect();
    inline void modbus_close() const;

    inline void modbus_set_slave_id(int id);

    inline int  modbus_read_coils(int address, int amount, bool* buffer);
    inline int  modbus_read_input_bits(int address, int amount, bool* buffer);
    inline int  modbus_read_holding_registers(int address, int amount, uint16_t *buffer);
    inline int  modbus_read_input_registers(int address, int amount, uint16_t *buffer);

    inline int  modbus_write_coil(int address, const bool& to_write);
    inline int  modbus_write_register(int address, const uint16_t& value);
    inline int  modbus_write_coils(int address, int amount, const bool *value);
    inline int  modbus_write_registers(int address, int amount, const uint16_t *value);
 ...
 */
inline void modbus::modbus_build_request(uint8_t *to_send, uint address, int func) const {
 ...
 */
inline int modbus::modbus_write(int address, uint amount, int func, const uint16_t *value) {
    int status = 0;
 ...
 * @param func      Modbus Functional Code
 */
inline int modbus::modbus_read(int address, uint amount, int func){
    uint8_t to_send[12];
    modbus_build_request(to_send, address, func);
    to_send[5] = 6;
    to_send[10] = (uint8_t) (amount >> 8u);
 ...
 */
inline int modbus::modbus_read_holding_registers(int address, int amount, uint16_t *buffer) {
    if(_connected) {
 ...
 * @param buffer      Buffer to Store Data Read from Registers
 */
inline int modbus::modbus_read_input_registers(int address, int amount, uint16_t *buffer) {
    if(_connected){
        if(amount > 65535 || address > 65535) {
            set_bad_input();
            return EX_BAD_DATA;
 ...
 */
inline int modbus::modbus_read_coils(int address, int amount, bool *buffer) {
    if(_connected) {
 ...
 */
inline int modbus::modbus_read_input_bits(int address, int amount, bool* buffer) {
    if(_connected) {
        if(amount > 2040 || address > 65535) {
            set_bad_input();
            return EX_BAD_DATA;
 ...
 */
inline int modbus::modbus_write_coil(int address, const bool& to_write) {
    if(_connected) {
        if(address > 65535) {
            set_bad_input();
            return EX_BAD_DATA;
 ...
 */
inline int modbus::modbus_write_register(int address, const uint16_t& value) {
    if(_connected) {
        if(address > 65535) {
            set_bad_input();
            return EX_BAD_DATA;
 ...
 */
inline int modbus::modbus_write_coils(int address, int amount, const bool *value) {
    if(_connected) {
 ...
 */
inline int modbus::modbus_write_registers(int address, int amount, const uint16_t *value) {
    if(_connected) {
        if(address > 65535 || amount > 65535) {
            set_bad_input();
            return EX_BAD_DATA;
